...
#data.cov1 = data.cov1/1e6
#data.cov2 = data.cov2/data.cov2.sum()

#for i in range(0, len(data)):
#  data.cov1.iloc[i] = rd.uniform(0, 0.999)
...
cais_kennedy = com_at_df.loc[[110, 116, 107, 113, 109, 21, 115, 111, 112, 104, 108, 105, 106, 13, 114]]

array_cais = np.array([])
array_data1 = np.array([])
for i in range(0, len(data)):
    for j in range(0, len(cais_kennedy)):
        array_cais = np.append(array_cais, data.geometry.iloc[i].distance(com_at_df.geometry.iloc[j])/1000)
    array_data1 = pd.Series(np.append(array_data1,min(array_cais)))
    
data.cov1 = (array_data1-array_data1.min())/(array_data1.max()-array_data1.min())
